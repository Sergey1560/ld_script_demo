...
void MPU_Config(void){
	...
	MPU->RBAR = (uint32_t)QSPI_BASE;
	MPU->RASR = ((uint32_t)MPU_INSTRUCTION_ACCESS_DISABLE << MPU_RASR_XN_Pos)   |
				((uint32_t)MPU_REGION_PRIV_RO  << MPU_RASR_AP_Pos)   |
				((uint32_t)MPU_TEX_LEVEL0          << MPU_RASR_TEX_Pos)  |
				((uint32_t)MPU_ACCESS_NOT_SHAREABLE << MPU_RASR_S_Pos)    |
				((uint32_t)MPU_ACCESS_CACHEABLE    << MPU_RASR_C_Pos)    |
				((uint32_t)MPU_ACCESS_NOT_BUFFERABLE << MPU_RASR_B_Pos)    |
				((uint32_t)0x0				       << MPU_RASR_SRD_Pos)  |
				((uint32_t)MPU_REGION_SIZE_4MB     << MPU_RASR_SIZE_Pos) |
				((uint32_t)1                       << MPU_RASR_ENABLE_Pos);
/*
	MPU->RNR = MPU_REGION_NUMBER2;
	MPU->RBAR = (uint32_t)0x30000000; //RAM D2
	MPU->RASR = ((uint32_t)MPU_INSTRUCTION_ACCESS_DISABLE << MPU_RASR_XN_Pos)   |
				((uint32_t)MPU_REGION_FULL_ACCESS  << MPU_RASR_AP_Pos)   |
				((uint32_t)MPU_TEX_LEVEL1          << MPU_RASR_TEX_Pos)  |
				...
				((uint32_t)MPU_ACCESS_BUFFERABLE << MPU_RASR_B_Pos)    |
				((uint32_t)0x0				       << MPU_RASR_SRD_Pos)  |
				((uint32_t)MPU_REGION_SIZE_512KB     << MPU_RASR_SIZE_Pos) |
				((uint32_t)1                       << MPU_RASR_ENABLE_Pos);
*/
